(() => {
	...
	newPost.addEventListener('submit', event => {
		...
		document.getElementById('new-post-text').innerHTML = '';
	...
	tweetsDB.on('child_added', data => {
		...
		newDiv.querySelector('.post-close').addEventListener('click', event => {
			const postID = event.target.getAttribure('data-id');
			tweet.delete(postId);
	...
	tweetsDB.on('child_changed', data => {
		...
		document.querySelectorAll('[data-id]').forEach(element => {
			if (element.getAttribute('data-id') == postId) {
				if (element.parentElement.getElementsByClassName('post-likes')[0]) {
					element.parentElement.getElementsByClassName('post-likes')[0].innerHTML = likes;
				}

				if (element.parentElement.getElementsByClassName('post-dislikes')[0]) {
					element.parentElement.getElementsByClassName('post-dislikes')[0].innerHTML = dislikes;
				}
			}
		})
		const currentUser = firebase.auth().currentUser;
